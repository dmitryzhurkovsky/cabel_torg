...
export default {
  ...
  actions: {
    ...
    try {
          const response = await axios.get(useRuntimeConfig().public.NUXT_APP_API_URL + "users/mine");
    ...
    async UPDATE_USER_REQUEST({ commit }, data) {
      commit("SET_ERRORS", {});
      try {
        const response = await axios.patch(useRuntimeConfig().public.NUXT_APP_API_URL + "users/mine", data);
        commit("SET_USER_DATA", response.data);
